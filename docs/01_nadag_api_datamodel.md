...
> **Coordinate provenance for soundings**: Both `GeotekniskBorehull` and `GeotekniskBorehullUnders` expose point geometry and elevation-like fields (`høyde`, `høydeReferanse`). In the current `nadag_python` output for soundings/methods, the exported `x`/`y` coordinates are derived from the `GeotekniskBorehull` (`locations`) geometry, and `z` is derived from `GeotekniskBorehull.høyde`. The `GeotekniskBorehullUnders` (`investigations`) geometry is only used as a fallback in metadata construction when a matching location geometry is not available. This means multiple investigations linked to the same borehole location may be represented at the same exported coordinate, even if the API exposes separate investigation geometries.
